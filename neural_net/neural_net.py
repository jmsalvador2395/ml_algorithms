...
class TwoLayerNN:
	def __init__(self, num_inputs, num_outputs):
		...
		self.h_matrix=np.random.randint(-500, 500, (self.h_size, num_inputs+1)) #+1 on num_inputs for the bias

		#set bias weight to 1 for input weight vectors
		self.h_matrix[:,0]=1

		#randomly initialize output layer weights
		self.v_matrix=np.random.randint(-500, 500, (num_outputs, self.h_size+1)) #+1 on h_size to account for bias
	...
	def forward_propogation(self, data):
		...
		hidden_output=np.array(self.h_size+1)
